...
open class NavigationListenerEmitter : NavigationListener() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        when (intent?.action) {
            SET_INTENT -> {
                mPendingIntent = if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.TIRAMISU) {
            ...
            CHECK_NOTIFICATIONS_ACCESS -> {
                ...
                try {
                    ...
                    Log.e("Lỗi gửi kết quả Access: $e")
                }
            }

            STOP_NAVIGATION -> {
                Log.d("Stopping navigation requested")
                val stopIntent = Intent(NAVIGATION_STOPPED)
